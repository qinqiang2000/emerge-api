...
walking-skeleton default (create_project + extract + agent_text).
...
async def stub_stream_turn(
    ...
    * ``/extract`` → extract-flow stub (list_docs + parallel extract).
    * else        → walking-skeleton default (create_project +
                    extract + ``Stub run complete``).
    """
    ...
    async def gen_extract():
        ...
        yield {"event": "tool_call", "data": json.dumps({
            ...
            "tool_name": "mcp__emerge_tools__extract",
