"""Audit judge — NL rules over a doc group, index-aligned (app/match/audit.py)."""
...
class _MockProvider:
    ...
    received (to assert image attachment)."""
...
_DOCS = {"quote": {"甲方": "环胜"}, "receipt": {"金额": "100"}}
...
async def test_aligned_verdicts():
    ...
    out = await audit_group(group_docs=_DOCS, audit_rules=_RULES, provider=p)
    assert [c.status for c in out] == ["pass", "fail", "pass"]
    assert [c.rule for c in out] == _RULES  # rule text preserved in order


async def test_missing_index_becomes_unclear():
    # judge omits rule 1's verdict → that rule stays unclear, others align
    p = _MockProvider(checks=[
...
async def test_no_provider_all_unclear():
    out = await audit_group(group_docs=_DOCS, audit_rules=_RULES, provider=None)
...
async def test_empty_rules_returns_empty():
    out = await audit_group(group_docs=_DOCS, audit_rules=[], provider=_MockProvider())
    assert out == []


async def test_anchor_image_attached_for_visual_rules():
    cap: dict = {}
    p = _MockProvider(checks=[{"index": 0, "status": "pass", "reason": "ok"}], capture=cap)
    img = ImageBlock(media_type="image/png", data_b64="c3R1Yg==")
    await audit_group(group_docs=_DOCS, audit_rules=["乙方盖红章"], anchor_image=img, provider=p)
    blocks = cap["user_content"]
    assert any(isinstance(b, ImageBlock) for b in blocks)   # image pulled in
    assert any(isinstance(b, TextBlock) for b in blocks)


async def test_no_image_when_none():
    cap: dict = {}
    p = _MockProvider(checks=[{"index": 0, "status": "pass", "reason": "ok"}], capture=cap)
    await audit_group(group_docs=_DOCS, audit_rules=["甲方为环胜"], anchor_image=None, provider=p)
    assert not any(isinstance(b, ImageBlock) for b in cap["user_content"])
