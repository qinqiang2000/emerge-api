...
// Analytics surfaces are URL-gated (`?eval` / `?bench` / compare route) and
// most sessions never open them — defer their chunks (each pulls heavy table /
// chart code) until the param actually appears.
  ...
  if (evalRoute && evalRoute.kind === 'compare') {
    return (
      <Suspense fallback={null}>
        <EvalCompare slug={evalRoute.slug} a={evalRoute.a} b={evalRoute.b} />
      </Suspense>
    )
  }
