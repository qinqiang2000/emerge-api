...
> **Status**: ✅ P0 (read 三件 ws_list/ws_read/ws_grep) + P1 关键部分 (add_model typed 工具 + skill interface-aware FS 分支) shipped & deployed prod (2026-06-09)；✅ **P1 写三件 ws_write/ws_edit/ws_move shipped (2026-06-10)**——签名**克隆 SDK 内置 Write/Edit**（`file_path/content`、`old_string/new_string/replace_all`），让模型对内置工具的肌肉记忆直接迁移；`schema.json` 在 path guard 服务端硬 block（红线:只走 write_schema）;`ws_move(copy=true)` 补二进制 doc 跨项目拷贝（ws_write text-only）;无 ws_delete。✅ **P2 audit (2026-06-10): read-collapse 结论=不可行**——Step B 已砍光纯 wrapper，幸存 read 工具各扛 env-fallback 解析(`get_*_config`)/聚合(`bench_view`)/迁移副作用(`list_projects`)/active 指针解析+global_notes 渲染契约(`read_prompt`)，全部保留;"收敛到 ~15"前提失效，大幅减 token 只剩 Tier-2 code-exec 路径;轻量选项=match/audit 套件 opt-in 开关(未做)。✅ **P2 OAuth dogfood 实锤 (2026-06-10)**——prod `_auth/oauth_clients.json` 有 DCR 注册的 `Claude Desktop (1.11187.4)` + 4 个已发 token,用户连接时的登录跳页即 OAuth 流,登录式 onboard 闭环。✅ **emerge_ 服务前缀 shipped (2026-06-10)**——headless 面 58 个工具名全部 `emerge_` 前缀(多 connector 防撞);annotation 桶/usage 遥测/浏览器 chat/HTTP/skill 源文保持裸名。✅ **写侧首条 dogfood (2026-06-10)**:「曹燕项目 Title→title」四步链 ws_grep→read_prompt→write_schema,invariant 写正确路由 typed,prod 落盘 version 2 + 快照。✅ **ws_edit/ws_move dogfood 全过 (2026-06-10)**:项目改名(ws_read→ws_edit 同步 project.json→ws_move,agent 比 skill 教的周全)+ 跨项目拷二进制 doc(ws_move copy=true,ctime 毫秒级对上 usage log;注意 copy2 保留 mtime,**验证拷贝用 ctime 不用 find -mmin**)。✅ **minimal surface 实验上线 (2026-06-10,prod 默认)**:`EMERGE_MCP_SURFACE=minimal` 把 headless 面 58→28——只列 ws_*(6)+invariant 写+LLM/job 动词;list_*/read_prompt/bench_view/set_*/match/audit 套件不进 tools/list(仍注册可调);`=full` 一键回退。**实验探针**:read_prompt 缺位后"看 prompt"质量(两步舞+global_notes 契约丢失)、audit/match 用户撞墙率、sidecar lazy-rebuild 缺 list_docs 触发点。skill 补 minimal 兜底段+headless 首调用叙述行(修 empty placeholder)。✅ **探针 1(read_prompt 缺位)满分 (2026-06-10)**:「曹燕的验证 prompt 是什么」→ 5 次 ws_list/ws_read(~34s)零撞墙;字段表带 description 分层渲染,**global_notes 7 条完整呈现**(契约没丢);**反转发现**:FS 路径主动报告了非激活历史 prompt(pr_jwt4wmqye50m)——typed read_prompt 给不出的信息,钥匙孔 vs 开门看全屋。成本=往返次数/延迟,质量未掉反升。✅ **探针 2 抓到真窟窿→当天补数据面 (2026-06-10)**:「创建百胜audit1+放入4个附件」→ create_project ✓ 但 4 个 PDF 在 **Cowork 沙箱**,服务器零摄入通道(full 面同样没有,非 minimal 砍出;'共享FS'假设第三次复发)。**根因固化:MCP 总线只有控制面没有数据面**——工具参数走模型 token 流,二进制(几MB base64)物理过不去;resources 也只有 server→client 方向。**修法=S3 模式**:`request_upload_url(slug, filenames)` mint HMAC capability URL(绑 team+slug+filename+15min TTL,token 即鉴权,agent 永不见 PAT/OAuth)→ agent 在自己沙箱 `curl --data-binary` 到无鉴权 `POST /lab/upload/{token}` → redeem 经 `upload_doc` 全套不变量(magic/dedupe/sidecar),重放只 dedupe 不覆盖。进 minimal 面(29)+twin `POST /lab/upload-urls`+skill 摄入分支。⚠️ **摄入 dogfood (2026-06-10):Cowork 沙箱出网被代理拦**——唯一外部依赖打穿。但 capability URL 的优雅降级生效:agent 自发把 curl 命令递给用户在本机终端执行(token 不绑客户端,**谁持有 URL 谁就是数据面**=人也行)。顺手修:token 内 ws 路径 mint 时 resolve 绝对(原相对路径依赖进程 cwd)。**降级阶梯**(待写进 skill 重打包):①沙箱 curl(若域名被加白)→②把 curl 命令交给用户本机执行→③emerge 浏览器 UI 上传。**Pending**: 查 Cowork 沙箱网络 allowlist 能否加白 fpydoc.duckdns.org(一次配置根治)+ minimal/full usage 遥测对比 + plugin 重打包(带 narration 修复+降级阶梯+瘦核心 275 行+路由纪律)。✅ 本机 curl 闭环已验(4 docs 落盘,页数/sha 正确)。
**Cowork computer-use dogfood (2026-06-11)**:① 远程审计全链通——ws_list 发现→run_audit→headless 契约满分渲染(7 条 ✓ 列表+整体结论)。② **幂等护栏在未预料场景立功**:Cowork 客户端工具超时(~60s)< 多页 judge 一趟(73s),首调"超时"但服务端跑完落盘;agent 重试命中 120s 幂等窗免费拿报告,还向用户如实说明"缓存结果(与上次一致)"——无护栏则超时重试再烧一趟可能再超时死循环。**Follow-up**:多页让 judge 趟时逼近/超过客户端超时,大组审计考虑 job 化(start_job/get_job 模式)或缩短趟时。③ 旧 plugin(868 行快照内嵌 audit 契约)下渲染未劣化、read_skill 未被调用(旧 skill 无路由表)——重打包后远程才真正走"瘦核心+read_skill"。④ empty placeholder 复现(旧 skill 无叙述修复)= 重打包实证。
