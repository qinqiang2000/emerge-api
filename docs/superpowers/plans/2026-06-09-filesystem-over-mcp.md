...
> **Status**: ✅ P0 (read 三件 ws_list/ws_read/ws_grep) + P1 关键部分 (add_model typed 工具 + skill interface-aware FS 分支) shipped & deployed prod (2026-06-09)；✅ **P1 写三件 ws_write/ws_edit/ws_move shipped (2026-06-10)**——签名**克隆 SDK 内置 Write/Edit**（`file_path/content`、`old_string/new_string/replace_all`），让模型对内置工具的肌肉记忆直接迁移；`schema.json` 在 path guard 服务端硬 block（红线:只走 write_schema）;`ws_move(copy=true)` 补二进制 doc 跨项目拷贝（ws_write text-only）;无 ws_delete。✅ **P2 audit (2026-06-10): read-collapse 结论=不可行**——Step B 已砍光纯 wrapper，幸存 read 工具各扛 env-fallback 解析(`get_*_config`)/聚合(`bench_view`)/迁移副作用(`list_projects`)/active 指针解析+global_notes 渲染契约(`read_prompt`)，全部保留;"收敛到 ~15"前提失效，大幅减 token 只剩 Tier-2 code-exec 路径;轻量选项=match/audit 套件 opt-in 开关(未做)。**Pending**: Cowork 重连重跑「增加 gemini-2.5-flash 然后提取」登录式 dogfood + 写侧 dogfood（远程改 prompt description 走 ws_edit）。
