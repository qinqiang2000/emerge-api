...
| **2026-06-03** — Users & Teams (多租户 auth 地基:team=workspace 前缀、cookie+PAT 双通道、superuser 建 team) | `2026-06-03-users-and-teams.md` | 📋 planned | 引入 user/team/superuser。**核心架构**:team 做成 workspace 子目录前缀(`workspace_root/teams/{tid}/{slug}/`),而非 `project.json` 加字段——隔离=物理目录(agent cwd 自动收紧)、`tools/`+`paths.py`+`chat/service.py` 零改,只在 27 个 route 文件把 `settings.workspace_root` 换成 `team_workspace` 依赖。auth 数据模型照搬 label-studio 概念(Organization→Team / token→invite_token / active_organization→active_team_id),FS 存 `_auth/users.json`+`teams.json`(flock+atomic),密码 argon2。**双通道鉴权(同事精神红线)**:浏览器走 starlette `SessionMiddleware` 长效 rolling cookie(关浏览器不重登,`max_age` 90 天);headless(Claude Code/Desktop cowork)走长效不过期 PAT `Authorization: Bearer`,`current_user` 同时接受 cookie 或 bearer,所有 `/lab/*` 对 headless 立即可用;`mcp_server.py` 经 `EMERGE_TEAM_ID`(+可选 PAT)选 team。决策:superuser 独占建 team(一客户一 team,成员转发同一邀请链接拉人,team 内无管理员);跨 team 项目共享**下一期**(复用 M9.4 fork);本期全套前端(login/signup + account 菜单 + Settings 含 PAT 管理 + superuser team 切换器)。prod fast-path `/v1/{pid}/extract` keystore entry 加 `team_id`。现有项目一次性幂等迁入 bootstrap team。Out:角色分级/邮箱验证码/OAuth/SSO/计费/用户自助建 team。 |

| **2026-06-08** — Cowork/Desktop remote MCP connector (emerge 作为 Streamable HTTP remote connector,多租户 per-request 路由) | `2026-06-08-cowork-remote-mcp.md` | ✅ P1 shipped (pending commit + ngrok→Cowork dogfood) | 「换前端=换 agent 客户端」远程落地。研究:Cowork=云 VM,只认 remote MCP(Streamable HTTP),stdio 够不着;remote 一次建,Desktop/Cowork/web/mobile 全亮。**P1(本次)**:`app/api/mcp_remote.py` — `RemoteMcpRegistry`(per-team-workspace lazy 缓存 + 常驻 task 持 `manager.run()` 生命周期,anyio task group 不跨任务) + `_authenticate`(三通道:`Bearer` header PAT / `?token=` query PAT / session cookie,**复用 `app.auth.deps._resolve_team_workspace`**,不重写 PAT 逻辑) + `make_mcp_asgi`(ASGI mount `/mcp`,disabled→404 / HTTPException→JSON / 委派 `handle_request`)。复用 `app.mcp_server.build_mcp_server`(stdio 与 remote 共用 filtered server:`ui_*`+`ask_user` 滤除、`emerge-extractor` prompt 暴露);`stateless=True`;默认开,`EMERGE_MCP_REMOTE=0` 关。tip 1 决策:**弃 stdio 的 `EMERGE_TEAM_ID` 单团队模型**,改 per-request team 路由(云端连入无 env)。tip 2 决策:`?token=` query PAT 让队友粘个人 connector URL 即可上手,**无需 OAuth**。Live smoke(in-process MCP client over Streamable HTTP):open mode 38 工具(43−5 filtered)/`ui_*`+`ask_user` 已滤/`emerge-extractor` prompt;tenant mode 无 token→401 / header PAT→38 工具(路由 team) / query PAT→38 工具。Tests `test_mcp_remote.py` 6 passed;symmetry+auth 30 passed;TestClient boot green。**P2** OAuth2+DCR(Cowork「Add connector」登录式 onboard,长杆);**P3** plugin 包(SKILL.md+.mcp.json+marketplace,`/emerge-*` slash);**P4** 工具收敛(38→~10 + per-tool policy)。tip 3:`mcp 1.27` 已够,P1 无需升级;依赖刷新建议独立变更放 P2 前。 |
| **2026-08-16** — 工具收敛 (Cowork remote MCP **P4**，从 `2026-06-08-cowork-remote-mcp.md` 独立出来) | `2026-08-16-tool-consolidation.md` | ✅ shipped | **先量数，6 月的「38→~10」两个数都已过期且方向是错的。**实测:源码 78 个 `@tool`,但**实际注册只有 69**——9 个(`delete_doc`/`rename_doc`/`rename_project`/`forget_memory`/`list_trash`/`restore_from_trash`/`history_*`)装了装饰器、有 HTTP 路由、有单测、skill 里也写着,却从没进 `create_sdk_mcp_server(tools=[...])`,对 chat/stdio/remote 三面全部不存在;单测绕过注册直接 import 模块函数,对称不变量又只扫源码文本,外加一条 Step B 时代的反向断言把 bug 钉在原地——**两个测试互相印证,两个都在量错的东西**。Task 1 修可达性 + 把不变量改成量运行时注册。然后按名词并 8 族(同名词·同输入形状·同 policy·成员无破坏性 op):`set_model(role=)`←4 · `get_project_config`←`get_labeler_config`(补回 `env_default`) · `extract(experiment_id?)`←2 · `score(kind=)`←3 · `control_job(action=)`←3 · `ui_focus(target=)`←4 · `render_board(kind=)`←2 · `history(op=)`←2。**可达 69→64,chat 59→54,headless listed 63→61,remote minimal 41→41(净不变)**——合并省的是 chat/full 面,remote 的 context 税早被 `_MINIMAL_SURFACE` 的 listing 收走了。破坏性 5 动词(`delete_project`/`delete_doc`/`freeze_version`/`issue_api_key`/`promote_experiment`)一律保持独立单名:**MCP annotation 按工具名,客户端 gate 只能按名字,服务端 `(noun,op)` policy 表传不过去**。本轮**不新增任何 cut**——剩下的零调用工具是罕见但正确的动词,砍它们正是 6 月按 suite 砍出事的地方。别名只做渲染侧(`legacyToolName.ts`,764 条历史 tool_call 存的是旧名),服务端不收旧名(沿用 `pre_label` 先例)。新增不变量:装饰器⊆注册、多 op 工具同 policy 桶且无破坏性成员、旧名不得残留在两棵 skill 树、表面尺寸回归钉。**踩到的坑见 INSIGHTS**:合并会塌陷下游按名字的派发(丢/白得两个方向,靠 `xxxKindOf` 解析器同时认新 kind 和历史旧名);jsonschema 在 handler 之前跑,enum 守卫是死代码;一个工具名平铺在 12-16 个地方(每族平均动 16 文件,`extract` 动 28)。 |

| **2026-06-11** — Audit Board (审核白板 + evidence 引文 + run_audit job 化 + Cowork 阶梯 + **B5b MCP Apps board**) | `2026-06-11-audit-board.md` | ✅ B0-B5b 全部 shipped + dogfooded | Seed `2026-06-11-audit-board-seed.md`。两 spike 已过：excalidraw 0.18.1 embed 可行（React 19 ✓ 多页铺板 60fps ✓ 双向联动 ✓；trap: regenerateIds:false / 属性面板岛 / IS_PREACT define / 低透明度填充）；MCP Apps 机制确认（`_meta.ui.resourceUri` + `ui://` 资源 mimeType `text/html;profile=mcp-app`，python mcp 1.27.2 全类型带 `_meta`，零新依赖）。Phases: B0 job 化 / B1 RuleCheck.evidence 纯文本引文 / B2 locate-quotes render 路由（非 @tool）/ B3 lab board（excalidraw）/ B4 服务端合成标注图（Cowork 兜底）/ B5a MCP Apps hello gate（Desktop 真机 = human dogfood）。P0.5 之 N-source 引擎已 shipped（engine.py 勘探确认），核对卡前端被 board 重塑（v1 audit-only，match board 留 follow-up）。 |

| **2026-08-20** — 对比交给产品经理（口径进产品：有值格硬指标 + 噪声护栏 + compare board + 无 GT 分歧裁决） | `2026-08-20-compare-for-pm.md` | ✅ shipped + deployed + dogfooded | 把 `~/Desktop/振兴_20260707_docs_and_results/_workbench/HANDOFF.md` 里手工积累的对比方法论拆成两半：**数字进报告，方法论进 skill**。P1 噪声护栏（skill only，双条件 Δ格数>3 且 Δpp>9 才允许下结论）；P2 `_aggregate` 补 wrong/missing/spurious 格级计数 → 有值格微平均 + 整篇零错 n/N 升为头条、官方 macro 降灰字，`SchemaField.required`（`extract.py:54` 证实是无消费者的死字段）激活为★重要字段档，报告做成 `render_board(kind='compare')` 第三种白板；P3 无 GT 时不 refuse 而是 `diff_predictions` 出分歧队列（复用 `normalize_equivalent`）→ chat 裁决 → `save_reviewed` 造 GT。红线：无 GT 的输出绝不出现百分比（一致率≠准确率）。**已交付** `6aded08` + `f16227d`（生产 smoke 抓到并修了「旧 blob 的 n/a 被误报成『先去 review』」——振兴有 638 格 GT 却被这么告知）。backend 1908 passed（净增 54），frontend tsc clean + 721 passed。T11 形态比 plan 更省：无 GT 不单开 kind，靠 `a`/`b` 形状分流（eval ts vs `_draft`/`ex_…`）。待 dogfood：振兴要看硬指标须重跑评测（现存 eval 全是 M12 blob），且其 `required` 只标了 4 个字段而非 HANDOFF 的 13 个。**浏览器 dogfood 已做**(`278f673`)：抓到 8 处,一半是单测原理上抓不到的(布局被长地址挤掉对面列、class 语义撞车导致 ★ 打错行、agent 明明有白板却不用)。最大的一条教训:**要 agent 给链接就给它链接模板,别要求它多调一次工具去取**;以及验证 skill 改动必须开新 chat(同一会话只在第一轮 read_skill)。**真实硬指标已跑**(`f3f3f2d`,零 LLM——run_experiment_eval 复用已有预测):gemini-2.5 vs gemini-3(同 Baseline v3,19 篇 GT)有值格 78.8%→86.0%(+7.2pp),而官方 macro 只差 +1.6pp——**送分格把差距稀释了 4 倍**,这张表本身就是本 milestone 的论据。判据判 noise(+7.2<9)与 HANDOFF 的四轮结论方向一致。terra 的 76.3%/83.9%/0-of-19 与 HANDOFF 手工计算逐位吻合。又抓到两 bug:同秒 eval 互相覆盖、判据缺「明显更差」态。|
