...
| **M11 Phase A** — turn as first-class resource (decouple SSE from turn lifetime; multi-client attach; "switch view doesn't kill the agent") | `2026-05-19-turn-as-resource.md` | ✅ Phase A shipped + dogfooded | `e9dd221..<t7>` (T0–T7). Live verified 2026-05-19: slow `/extract` turn on 太古_美国发票, switched to 荣耀_欧洲1 mid-stream — localStorage `turn:cid` preserved; backend `turn_state` still `running` with `last_offset` advancing; switched back → re-attach fired, full extract result rendered. Phase B follow-ups: 503-on-cold-cache reattach (cosmetic); 13 tool-only HTTP fillers (T8–T14). |
