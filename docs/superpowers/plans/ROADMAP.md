...
- **`issue_api_key` agent re-renders a key-info markdown table** — surfaced during the 2026-05-11 post-M7.1 end-to-end verify. Same anti-pattern as M7.1 T7 (agent duplicates a structured tool result the UI already renders), but for the `issue_api_key` tool, which T7 didn't cover. Fix is a 3-line "Rendering contract" addition to `emerge_publish.md` step 7. **M7.2 candidate.** See the new 2026-05-11 entry in `docs/design-decisions.md`.
