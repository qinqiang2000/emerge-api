...
| **2026-08-20** — 对比交给产品经理（口径进产品：有值格硬指标 + 噪声护栏 + compare board + 无 GT 分歧裁决） | `2026-08-20-compare-for-pm.md` | 📋 planned | 把 `~/Desktop/振兴_20260707_docs_and_results/_workbench/HANDOFF.md` 里手工积累的对比方法论拆成两半：**数字进报告，方法论进 skill**。P1 噪声护栏（skill only，双条件 Δ格数>3 且 Δpp>9 才允许下结论）；P2 `_aggregate` 补 wrong/missing/spurious 格级计数 → 有值格微平均 + 整篇零错 n/N 升为头条、官方 macro 降灰字，`SchemaField.required`（`extract.py:54` 证实是无消费者的死字段）激活为★重要字段档，报告做成 `render_board(kind='compare')` 第三种白板；P3 无 GT 时不 refuse 而是 `diff_predictions` 出分歧队列（复用 `normalize_equivalent`）→ chat 裁决 → `save_reviewed` 造 GT。红线：无 GT 的输出绝不出现百分比（一致率≠准确率）。|
