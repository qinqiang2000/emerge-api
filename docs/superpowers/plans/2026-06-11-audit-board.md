...
> **Status**: ✅ B0-B5a code landed 2026-06-11（backend 1560 passed；前端 +43，14 失败为既有 scrollIntoView）。**Live dogfood**：① B3 board ✅ prod 真实数据两轮过（百胜audit1，7 规则 29 证据 23 页）。一轮修 fit 失效/长条布局/怼脸 zoom（`ec7893e`）；二轮按用户反馈定型交互（`3998cd8`）：**store 自愈暖层**（miss 的 (doc,page) 走 GET /textlayer 暖 OCR 再重定位——报价单.pdf 实为扫描件，暖后引文全命中，"未命中"质量问题就此关闭）、**单规则圈注**（场景只铺页图，点行按需挂该规则虚线圈+连线，全画太乱）、虚线描边无填充、行点击兜底跳文档页、文档按页数升序排（跨文档规则相邻）、全屏。截图 `docs/screenshots/2026-06-11-board-*`；② B4 Cowork 圈图 ✅（跑通但发现 **Cowork 不内联渲染工具图**——图给模型不给用户;补了服务端自暖+虚线样式对齐+board 深链,`0229b5e..`）；③→B5b 直接落地（跳过 hello gate）：`ui://emerge/audit-board.html` 单文件 SVG 白板 + HMAC capability 数据面（`board_view.py` + `/lab/board-view/{token}` 双格式:浏览器→白板/fetch→JSON）+ `_meta.ui` 注入。**服务端全链验证 ✅**（prod tools/list 实测带 `_meta.ui`;standalone 白板真机圈+连线+联动全过,截图 `2026-06-11-appsboard-*`）。**iframe 就地渲染 blocked on 客户端面**：用户 Desktop=第三方 gateway 的 Cowork 专用面（无官方 chat）,网页=Free plan（无 connector）——均不渲染 Apps（终验 2026-06-11 晚:Cowork 真机调 read_audit_report 仍纯文字,服务端 _meta 同时直测在发）;官方付费面接入时零改动可用。当下完成态=Cowork 给 board 深链,点开即交互白板（同一 capability URL 双格式）。诊断器留存:`mcp_server.py` list_tools 时 log `session.client_params`(clientInfo+capabilities,挂 uvicorn.error)。
