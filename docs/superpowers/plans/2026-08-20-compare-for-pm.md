...
## 5.7 真实硬指标（2026-08-21，`f3f3f2d` + 表头改名）

前一轮 dogfood 只验到了降级路径 —— **plan 的头号目标（硬指标 + 判据）一次都没在
真实数据上跑出来过**。这一轮补上了，而且**零 LLM 成本**：`run_experiment_eval`
对已有预测是直接复用的（`experiment.py` 的 `ep.exists()` 分支），振兴的 4 个实验
各有 19 篇预测，重打分只要 ~200ms。

`gemini-2.5-flash · Baseline v3` vs `gemini-3-flash-preview (low) · Baseline v3`
（同一份 prompt，公平对比，19 篇 GT）：

| 口径 | gemini-2.5 | gemini-3 (low) | Δ |
|---|---|---|---|
| ★重要字段 · 有值格 | 85.9% | 91.0% | +5.1pp |
| 全字段 · 有值格 | 78.8% | 86.0% | **+7.2pp**（+26 格 / 363 格） |
| 整篇零错文档 | 1/19 | 1/19 | +0 |
| 官方 macro | 87.0% | 88.6% | +1.6pp |

**这张表本身就是 plan 的论据**：官方 macro 只差 **+1.6pp**，有值格差 **+7.2pp** ——
送分格把真实差距稀释了 4 倍多。PM 只看 macro 会得出「几乎一样」，那正是这个
milestone 要防的误判。

判据判 **noise**（+7.2pp < 9pp）→「分不出高下，维持现状」。**这是对的**：
HANDOFF 当年也是靠四轮区间不重叠才敢说 gemini-3 赢，单轮 +5.3pp 落在噪声带内。
产品比 HANDOFF 保守，且方向一致。

**三个数字与 HANDOFF 独立手工计算逐位吻合**，是最强的口径正确性验证：
terra 全字段有值格 **76.3%**、官方 macro **83.9%**、整篇零错 **0/19**；
连有值格分母 **363** 都一样。

**逐字段表还暴露了 HANDOFF 没提的一条**：gemini-3 并非全面领先 ——
`totalTaxAmount` **−16.7pp**、`billFromTaxIdentificationNumber` **−14.3pp**
（两个都是 100% → 83.3%/85.7%）。整体赢、局部退，这正是逐字段表存在的理由。

这一轮又抓到两个真 bug（`f3f3f2d`）：**同秒 eval 互相覆盖**（重打分只要 200ms，
三个模型全落在同一秒，后两个把前面的结果覆盖掉且无声无息）；**判据缺
「挑战者明显更差」这一态**（拿差模型来比会得到「分不出高下」—— 动作没错，
但那句话是错的）。

**表头改名**：总体表和逐字段表的列名从「在位 / 挑战者」换成模型 × prompt 的真名。
标题里已经写了「A vs B」，表格再用角色词就要读者在两处之间做映射 —— 而这份报告
的读者正是不熟悉这套术语的人。

## 5.8 逐格明细（2026-08-21）—— 汇总回答不了「哪几篇」

对照用户手工那份 `compare_gemini3low_vs_gemini25.html` 发现的缺口：那份报告里
字段名可以点开，展开该字段**每一处失分的格级明细**，每行的文档名还是链到复核页
的链接。我的版本只到字段级汇总 —— PM 看到一行 `-16.7pp` 之后必然问「哪几篇、
错成什么」，报告就断在这里。

补上后（读 `cells.jsonl`，两侧按 `(filename, entity_idx, field)` join）：

- 逐字段表的字段名带上「N」并成为页内锚点，**只在真有问题格时才做链接**
  （没有明细的字段点开是空的，反而误导）
- 每个字段一个 `<details>`：只列**至少一侧判错**的格（两侧都对的没信息量），
  列序 `文档 | GT（正确值） | <模型A> | 它给了什么 | <模型B> | 它给了什么`
- **每块自带表头** —— 展开某一块时别处的列说明是看不到的
- 判定用中文（对 / 错值 / 漏抽 / 多填 / 都空），不是 `wrong`/`spurious`
- 文档名链到 `?review=<filename>`，`target="_blank"`

**iframe 的 `sandbox=""` 会把 `target="_blank"` 一起挡掉**（点了毫无反应），
所以放开 `allow-popups allow-popups-to-escape-sandbox` —— 脚本、表单、同源仍全禁。

实测振兴那份：**112 个文档链接、24 个 drill 块、74KB**。而它立刻给出了汇总给不了
的判断：`totalTaxAmount` 的 −16.7pp 里只有 1 格是真错
（`20260707105737.pdf`，GT `44.54` → gemini-3 给 `0.0`，把税额抹成零），
其余 4 格是「多填 ↔ 都空」的互换。**换模型该不该因为这一行犹豫，看汇总答不了，
看明细一眼就有答案。**

顺带一个自找的坑：CSS 里的 `content: "\25B8"` 经 Python 字符串落盘后没能被 CSS
正确解析，页面上每行前面显示字面量 `B8`。直接写字符 `▸` —— 少一层可以出错的转换。
